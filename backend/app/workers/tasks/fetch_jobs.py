...
async def _fetch_jobs_from_source_async(source_id: UUID) -> dict:
    ...
    async with async_session_maker() as session:
        try:
            ...
            # Initialize provider based on source type
            if source.name.lower() == "headhunter_kz" or source.type == "hh_kz":
                provider = HeadHunterKZProvider(source.configuration)
            else:
                logger.error(f"Unknown source type: {source.type}")
                return {"status": "error", "message": f"Unknown source type: {source.type}"}
